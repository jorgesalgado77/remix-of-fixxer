...
// Usando as variáveis que o sandbox injeta
...
async function runAudit() {
  ...
  const { data: profileSample, error: pError } = await supabase.from('profiles').select('id, full_name, avatar_url').limit(5);
  if (profileSample && profileSample.length > 0) {
    report.push("🟢 PASS: Perfis reais encontrados no banco.");
  } else {
    report.push(pError ? `🔴 FAIL: ${pError.message}` : "🟡 PARTIAL: Nenhum perfil real para validar.");
  }
  ...
  if (feedError) {
    ...
    report.push(feedData && feedData.length > 0 ? "🟢 PASS: Feed possui dados reais." : "🟡 PARTIAL: Feed está vazio.");
  }

  report.push("", "## 4. CHAT & SEGURANÇA (PROMPT 23)");
  const { error: blockError } = await supabase.from('user_blocks').select('id').limit(1);
