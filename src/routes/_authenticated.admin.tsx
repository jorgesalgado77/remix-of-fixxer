...
export const Route = createFileRoute("/_authenticated/admin")({
  beforeLoad: async () => {
    // Tenta bypass precoce se a flag estiver no localStorage e for CATEGORIA ADMIN
    if (typeof window !== 'undefined' && 
        localStorage.getItem('fixxer:master-bypass') === 'true' && 
        localStorage.getItem('fixxer:last-category') === 'admin') {
      console.warn("[Route Guard] Bypass Master ADMIN detectado no beforeLoad de /admin");
    ...
    <div className="p-4 md:p-8 space-y-8 animate-in fade-in duration-500 max-w-7xl mx-auto pb-24">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <Link to="/profile" search={{ focus: "" } as any} className="p-3 rounded-2xl bg-white/5 border border-white/10 text-white hover:bg-white/10 transition-all">
