...
function FeedPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "demandas_lojista" | "obras_b2c" | "prestadores" | "parceiros"
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 max-w-5xl mx-auto pb-24 animate-in fade-in duration-500">
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-col gap-3">
            <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
              <span className="shrink-0 text-[8px] font-bold text-primary/50 uppercase self-center mr-1">
                Técnico:
              </span>
            ...
            <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
              <span className="shrink-0 text-[8px] font-bold text-blue-400/50 uppercase self-center mr-1">
                Obras:
              </span>
            ...
            <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
              <span className="shrink-0 text-[8px] font-bold text-emerald-400/50 uppercase self-center mr-1">
                Fornecedores:
              </span>
      ...
      <div id="ts-visual-edit-probe-be9d757a1160472a" className="hidden">
        REGRAS ESTRITAS DE EXECUÇÃO E ESTABILIDADE: 1. PROIBIDO DESTRUIR OU REESCREVER CÓDIGOS E
        COMPONENTES QUE JÁ ESTÃO FUNCIONANDO. 2. EXECUTAR EXCLUSIVAMENTE O QUE FOI SOLICITADO NESTE
        PROMPT. 3. OTIMIZAÇÃO OBRIGATÓRIA PARA MOBILE: Modais leves de ação rápida e responsividade
        total. 4. SCROLL INVISÍVEL CORRETO E SEM PERDA DE FOCO EM COMPONENTES TOUCH. --- TAREFA:
        MODAL DE REPUTAÇÃO (0 A 5 ESTRELAS) E CARTEIRA COM RETENÇÃO SEGURA 1. SISTEMA DE AVALIAÇÃO
        MÚTUA (0.0 A 5.0 ESTRELAS): - Crie o componente modal de avaliação pós-serviço: * Lojista
        avalia Prestador (Pontualidade, Qualidade e Limpeza). * Prestador avalia Lojista/Cliente
        (Informaçoes e Recepção). * Cliente avalia Prestador (Cordilidade e Acabamento). - Exiba a
        média das notas nos cards dos perfis com o "Selo Ouro FIXXER" para médias acima de 4.8. 2.
        CUSTÓDIA DE PAGAMENTO PROTEGIDO (ESCROW SYSTEM): - Adicione o badge visual "Pagamento em
        Custódia Protegida FIXXER" nos detalhes da O.S. - Indicação clara de que o saldo só é
        liberado mediante comprovação fotográfica do serviço concluído. --- AUDITORIA GLOBAL DO
        SISTEMA (RELATÓRIO OBRIGATÓRIO): Ao concluir, passe o scanner no projeto inteiro e
        apresente: 1. Resumo do que foi criado e mantido com sucesso. 2. Confirmação de que nenhuma
        funcionalidade antiga foi danificada. 3. Status final do build (zerado de erros de rotas ou
        telas brancas).
      </div>
      ...
      <main className="space-y-4">
        {isLoading ? (
          <div className="space-y-4">
            {[1, 2, 3].map((i) => (
              <div key={i} className={`h-40 rounded-3xl animate-pulse ${glassClass}`} />
            ))}
          </div>
        ) : !posts.length ? (
          ...
          <>
            ...
            <div ref={ref} className="py-4 text-center">
              {isFetchingNextPage && (
                <div
                  className={`h-10 w-10 border-2 border-[#00FF87] border-t-transparent rounded-full animate-spin mx-auto`}
                ></div>
              )}
            </div>
          </>
...
function FeedCard({
  ...
  const maskContacts = (text: string) => {
    if (!text) return "";
    return text
      .replace(/\(?\d{2}\)?\s?\d{4,5}-?\d{4}/g, "[CONTATO PROTEGIDO]")
  ...
  return (
    <div
      className={`p-5 rounded-3xl border border-white/5 ${glassClass} hover:border-[#00FF87]/30 transition-all group relative overflow-hidden`}
    >
            ...
            <div className="flex items-center gap-2">
              ...
              {isVitrine && (
                <span className="text-[8px] font-black text-amber-500 flex items-center gap-1">
                  ...
                  {profile?.karma_score ? Number(profile.karma_score).toFixed(1) : "5.0"}
                </span>
              )}
            </div>
          </div>
      ...
      <DialogContent className="bg-[#0A0A0B] border-white/10 text-white">
        <DialogHeader>
          <DialogTitle className="text-white uppercase italic font-black">
            Enviar {post.feed_type === "Demanda_Cliente" ? "Orçamento" : "Proposta"}
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-4 py-4">
          ...
          <div className="p-3 rounded-xl bg-blue-500/5 border border-blue-500/10">
            <p className="text-[9px] text-blue-400 font-bold uppercase leading-tight">
              ...
              Atenção: É proibido o compartilhamento de contatos antes da aceitação da proposta. O
              descumprimento pode levar ao banimento.
            </p>
          </div>
        </div>
        <DialogFooter>
